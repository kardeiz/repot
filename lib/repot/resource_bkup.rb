...
#        public_attribute_set_wrapped.each_with_object({}) do |_attr, acc|
#          val = object.__send__(_attr.name)
#          acc[_attr.name.to_s] = if _attr.is_association? && !val.blank?
#            if _attr.is_collection?
#              val.map{|x| yield x, wrapper }
#            else yield val, wrapper end
#          else val end
#        end
